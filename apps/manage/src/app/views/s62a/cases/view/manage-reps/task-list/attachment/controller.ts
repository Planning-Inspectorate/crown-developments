...
export function buildReviewDocumentDecision(service: ManageService, journeyId: string) {
	...
	return async (
		...
		if (redactedFile) {
			...
			if (journeyId === MANAGE_REPS_MANAGE_JOURNEY_ID) {
				...
				// await deleteDocumentFromSharePointById(
				//     req,
				//     sharePointDrive,
				//     logger,
				//     redactedFile.itemId || redactedFile.id
				// );
			}
		}
...
export function buildDeleteRepresentationRedactedDocumentMiddleware(journeyId: string): RequestHandler {
	return (req, res, next) => {
		if (journeyId === 's62a-manage-reps-manage-journey') {
			const { representationRef, documentId } = getStringParams(req.params, ['representationRef', 'documentId']);
