...
describe('buildViewCaseFolders', () => {
	...
	beforeEach(() => {
		mockDb.s62aCase.findUnique.mock.resetCalls();
	...
	describe('Happy Path', () => {
		it('should fetch data and render the view', async () => {
			...
			assert.deepStrictEqual(mockDb.folder.findMany.mock.calls[0].arguments[0], {
				where: { s62aCaseId: 'case-123', deletedAt: null, parentFolderId: null }
			});

			assert.strictEqual(res.render.mock.callCount(), 1);

			const [viewPath, viewData] = res.render.mock.calls[0].arguments;
			assert.strictEqual(viewPath, 'views/s62a/cases/view/folders/view.njk');
			assert.strictEqual(viewData.pageHeading, 'REF-001');
			assert.strictEqual(viewData.folders.length, 2);
			assert.strictEqual(viewData.backLinkUrl, '/s62a/cases/case-123/overview');
		});
	});
	...
	describe('Error Handling', () => {
		it('should trigger Not Found logic if Case or Folders are missing', async () => {
			...
			mockDb.s62aCase.findUnique.mock.mockImplementation(() => Promise.resolve(null));
		...
		it('should propagate generic DB errors', async () => {
			...
			mockDb.s62aCase.findUnique.mock.mockImplementation(() => Promise.reject(dbError));
			mockDb.folder.findMany.mock.mockImplementation(() => Promise.resolve([]));
