...
export const crownEditsToDatabaseUpdates: ApplyCaseTypeUpdates = (updateInput, edits, viewModel, options) => {
	...
	if ('applicantContactId' in edits) {
		updateInput.ApplicantContact = edits.applicantContactId
			? { connect: { id: edits.applicantContactId } }
			: { disconnect: true };
	}
	// TODO Remove in CROWN-1509
	if ('agentContactId' in edits) {
		updateInput.AgentContact = edits.agentContactId ? { connect: { id: edits.agentContactId } } : { disconnect: true };
	}
