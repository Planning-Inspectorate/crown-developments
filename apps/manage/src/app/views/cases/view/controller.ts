...
import type { BannerMessage } from '@pins/crowndev-lib/views/banner/banner-builder.ts';
...
/**
 * Get any banner messages to show on the case details page.
 */
async function getBannerMessages(
	...
	const linkedCaseLink = await maybeGetLinkedCaseLink(db, crownDevelopment);
	if (linkedCaseLink) {
