...
describe('publish case', () => {
	describe('buildGetValidatedCaseMiddleware', () => {
		...
		it('should call db.crownDevelopment.findUnique with the correct id', async () => {
			...
			assert.strictEqual(mockDb.crownDevelopment.findUnique.mock.calls.length, 1);
			console.log(mockDb.crownDevelopment.findUnique.mock.calls[0][0]);
		...
		it('should call add an error to session for each required field missing and redirect to case page', async () => {
			...
			assert.strictEqual(req.session.cases['id-1'].errors.length, 4);
	...
	describe('publishSuccessfulController', () => {
		it('should render the correct view', async () => {
			const mockReq = {
				params: { id: 'case-1' },
				session: { cases: { 'case-1': { reference: 'ref-1' } } }
			};
			const mockRes = {
				locals: {},
				render: mock.fn()
