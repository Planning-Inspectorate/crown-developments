...
} from '@pins/crowndev-lib/forms/representations/questions.js';
	...
	return {
		async reviewRepresentation(req, res) {
			...
			if (reviewDecision !== ACCEPT_AND_REDACT) {
				repUpdate.status = reviewDecision;
			}
		...
		async redactRepresentation(req, res) {
			...
			const representation = await db.representation.findUnique({
				where: { reference: repId },
				select: { originalComment: true }
			});

			const response = new JourneyResponse(JOURNEY_ID, 'ref-1', {
				comment: representation.originalComment,
				commentRedacted: readSessionData(req, repId, 'redactedComment', '', 'representations')
			});
		...
		async redactRepresentationPost(req, res) {
			...
			addSessionData(req, repId, { redactedComment: comment }, 'representations');
		...
		async redactCheckYourAnswers(req, res) {
			const { repId } = validateParams(req.params);
			const redactedComment = readSessionData(req, repId, 'redactedComment', '', 'representations');
		...
		async acceptRedactedComment(req, res) {
			const { id, repId } = validateParams(req.params);
			const comment = readSessionData(req, repId, 'comment', '', 'representations');
			await db.representation.update({
				where: { reference: repId },
				data: {
...
function representationToViewModel(reference, representation) {
	return {
		...
		submittingFor: submittingFor(representation.representedTypeId),
		fullName: representation.Contact?.fullName,
		email: representation.Contact?.email,
		comment: representation.originalComment,
		redactedComment: representation.redactedComment,
		wantsToBeHeard: booleanToYesNoOrNull(representation.wantsToBeHeard),
		isAdult: booleanToYesNoOrNull(representation.isAdult),
		requiresReview: representation.status === REPRESENTATION_STATUS_ID.AWAITING_REVIEW
	};
}
 ...
 * @returns {string}
 */
function submittingFor(answer) {
	switch (answer) {
		case REPRESENTATION_TYPE_ID.PERSON:
			return SUBMITTING_FOR_OPTIONS.MYSELF;
		case REPRESENTATION_TYPE_ID.FAMILY_GROUP:
			return SUBMITTING_FOR_OPTIONS.AGENT;
		case REPRESENTATION_TYPE_ID.ORGANISATION:
			return SUBMITTING_FOR_OPTIONS.ORGANISATION;
	}
	return null;
}
