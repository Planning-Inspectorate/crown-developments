...
import {
	...
	clearRepUpdatedSession,
	readRepUpdatedSession,
...
describe('controller', () => {
	...
	describe('reviewRepresentation', () => {
		...
		it('should handle accept/reject', async () => {
			...
			assert.strictEqual(mockReq.session?.cases['case-1'].representationUpdated, 'accepted');
	...
	describe('redactCheckYourAnswers', () => {
		it('should render check-your-answers page with redacted comment from session', async () => {
			...
			await redactCheckYourAnswers(mockReq, mockRes);
			assert.strictEqual(mockRes.render.mock.callCount(), 1);
	...
	describe('session functions', () => {
		describe('readRepUpdatedSession', () => {
			it('should read session data', () => {
				const mockReq = { session: { cases: { 'case-1': { representationUpdated: 'accepted' } } } };
				assert.strictEqual(readRepUpdatedSession(mockReq, 'case-1'), 'accepted');
			});
		});
		describe('clearRepUpdatedSession', () => {
			it('should clear session data', () => {
				const mockReq = { session: { cases: { 'case-1': { representationUpdated: 'accepted' } } } };
				clearRepUpdatedSession(mockReq, 'case-1');
				assert.strictEqual(mockReq.session.cases['case-1'].representationUpdated, undefined);
			});
		});
	});
});
