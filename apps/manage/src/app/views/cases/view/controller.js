...
import { dateIsInThePast, dateIsToday } from '@pins/dynamic-forms/src/lib/date-utils.js';
...
export async function viewCaseDetails(req, res) {
	...
	const errors = readSessionData(req, id, 'errors', [], 'cases');
	if (errors.length > 0) {
		res.locals.errorSummary = errors;
	}
	clearSessionData(req, id, 'errors', 'cases');
	...
	const casePublished = publishDate && (dateIsToday(publishDate) || dateIsInThePast(publishDate));
