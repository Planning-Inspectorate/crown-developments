	...
	describe('getIsRepresentationWindowOpen', () => {
		...
		describe('representation dates handling', () => {
			...
			for (const test of tests) {
				...
				it(`should ${test.expected ? '404' : 'go to next'} when ${test.testName}`, async () => {
					const isRepresentationWindowOpen = getIsRepresentationWindowOpen(mockDb);
					const epochNow = Date.parse(test.inputs.now);
					mock.timers.setTime(epochNow);
					if (test.expected) {
						await assertRenders404Page(isRepresentationWindowOpen, mockReq, true);
					} else {
						const next = mock.fn();
		...
		it('should 404 when representationsPeriodStartDate is undefined', async () => {
			const mockReq = { params: { applicationId: 'cfe3dc29-1f63-45e6-81dd-da8183842bf8' } };
			const mockDb = {
				crownDevelopment: {
					findUnique: mock.fn(() => ({
