import { formatDateForDisplay, todayIsWithinRange } from '@pins/dynamic-forms/src/lib/date-utils.js';
...
/**
 * @param {string} id
 * @param { Object<end: Date | string, start: Date | string> } haveYourSayStartAndEndDates
 * @returns {import('./types.js').ApplicationLink[]}
 */
export function applicationLinks(id, haveYourSayStartAndEndDates) {
	const links = [
		{
	...
	if (todayIsWithinRange(haveYourSayStartAndEndDates?.start, haveYourSayStartAndEndDates?.end)) {
		links.push({
