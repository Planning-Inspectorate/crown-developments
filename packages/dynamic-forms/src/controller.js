...
/**
 * @param {(params: SaveParams) => Promise<void>} saveImpl
 * @returns {import('express').Handler}
 */
export function buildSave(saveImpl) {
	return async (req, res) => {
		...
		try {
			...
			const errorViewModel = questionObj.checkForValidationErrors(req, section, journey);
			...
			const saveViewModel = questionObj.checkForSavingErrors(req, section, journey);
			if (saveViewModel) {
				return questionObj.renderAction(res, saveViewModel);
			}

			// move to the next question
			return questionObj.handleNextQuestion(res, journey, section.segment, questionObj.fieldName);
		} catch (err) {
			const viewModel = questionObj.prepQuestionForRendering(sectionObj, journey, {
